...
pair<double, double> RootMethods::findBracket(const ex &f_expr, symbol x, double start, double end, double step)
{
    double prev_x = start;
    double prev_f;
...
RootReturn RootMethods::newtonMethod(const ex &f_expr, symbol x, pair<double, double> &bracket, double tol, int maxIterations)
{
    RootReturn History;
...
RootReturn RootMethods::bisectionMethod(const ex &f_expr, symbol x, pair<double, double> &bracket, double tol, int maxIterations)
{
    RootReturn History;
    ...
    for (int i = 0; i < maxIterations; ++i) {
        ...
        History.RootVariables['x'].push_back((History.RootVariables['a'].back() + History.RootVariables['b'].back()) / 2.0);
...
RootReturn RootMethods::secantMethod(const ex &f_expr, symbol x, pair<double, double> &bracket, double tol, int maxIterations)
{
    RootReturn History;
    ...
    for (int i = 2; i < maxIterations; i++)
    {
        ...
        {
            if(i > 2) return History; else return {};
        }

        History.RootVariables['x'].push_back(History.RootVariables['x'][i-1] - (f_x1 * (History.RootVariables['x'][i-1] - History.RootVariables['x'][i-2]) / (f_x1 - f_x0)));
